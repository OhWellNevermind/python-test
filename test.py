...
label = customtkinter.CTkLabel(master=generate_voice, text="Generate Voice", font=("Roboto", 24, 'bold'))
label.pack(pady=15, padx=10)

def script_upload():
    global script_location
    script_location = filedialog.askopenfilename(title='Upload Script', filetypes=(('Text Files', '*.txt'),), initialdir='./')
...
def voice_generation():
    ...
    for i, phrase in enumerate(script):
        ...
        if progressValue == 100:
            tkinter.messagebox.showinfo(title='Done', message='Voice was generated')
...
file_entry = customtkinter.CTkEntry(master=generate_voice, width=270, font=("Roboto", 15))
file_entry.configure(state=DISABLED)
file_entry.pack(pady=10)

upload_script = customtkinter.CTkButton(master=generate_voice, text='Upload Script', font=("Roboto", 15), command=script_upload)
upload_script.pack(pady=10)

audios_entry = customtkinter.CTkEntry(master=generate_voice, width=270, font=("Roboto", 15))
audios_entry.configure(state=DISABLED)
audios_entry.pack(pady=10)

audio_location = customtkinter.CTkButton(master=generate_voice, text='Set Location', font=("Roboto", 15), command=set_location)
audio_location.pack()

voice_label = customtkinter.CTkLabel(master=generate_voice, text="Select Voice", font=("Roboto", 15, 'bold'))
voice_label.pack(pady=10)

voice_menu = customtkinter.CTkComboBox(master=generate_voice, width=270, values=list(voices.keys()))
voice_menu.pack(pady=10)

def start():
    if not script_location or not audios_location:
        tkinter.messagebox.showerror(title='Error', message='Please set script location and save directory')
        return
...
start_btn = customtkinter.CTkButton(master=generate_voice, text='Start', font=("Roboto", 20, 'bold'), width=220, height=40, command=start)
start_btn.pack(pady=25)

progressbar = Progressbar(master=generate_voice, length=300, value=0)
progressbar.pack(pady=8, padx=8)

progress = customtkinter.CTkLabel(master=generate_voice, text='0%', font=("Roboto", 16, 'bold'))
...
def increase_size():
    for i, image in enumerate((os.listdir(images_location))):
        ...
        new_width, new_height = int(width * image_size), int(height * image_size)

        resized_img = img.resize((new_width, new_height))
        resized_img.save(img_path)

        progressValue = round(((i + 1) / len(os.listdir(images_location))) * 100, 1)

        images_progressbar.configure(value=progressValue)
        images_progress.configure(text=f'{progressValue}%')

        if progressValue == 100:
            tkinter.messagebox.showinfo(title='Done', message='Images size was increased')


size_label = customtkinter.CTkLabel(master=increase_img_size, text="Increase Image Size", font=("Roboto", 24, 'bold'))
size_label.pack(pady=15, padx=10)

image_entry = customtkinter.CTkEntry(master=increase_img_size, width=270, font=("Roboto", 15))
image_entry.configure(state=DISABLED)
image_entry.pack(pady=10)

image_location = customtkinter.CTkButton(master=increase_img_size, text='Set Location', font=("Roboto", 15), command=get_images_location)
image_location.pack(pady=10)

def images_start():
    if not images_location:
        tkinter.messagebox.showerror(title='Error', message='Please set images location')
        return
...
size_slider_label = customtkinter.CTkLabel(master=increase_img_size, text="Size: 1.25x", font=("Roboto", 15, 'bold'))
size_slider_label.pack(pady=6)

size_slider = customtkinter.CTkSlider(master=increase_img_size, from_=1, to=1.5, number_of_steps=10, command=set_size)
size_slider.pack()
size_slider.set(1.25)

images_start_btn = customtkinter.CTkButton(master=increase_img_size, text='Start', font=("Roboto", 20, 'bold'), width=220, height=40, command=images_start)
...
images_progress = customtkinter.CTkLabel(master=increase_img_size, text='0%', font=("Roboto", 16, 'bold'))
...
def delete_text():
    images = natsorted(list(map(get_images_path, os.listdir(clear_images_location))))
    ...
    for index, image in enumerate(images):
        ...
        progressValue = round(((index + 1) / len(os.listdir(clear_images_location))) * 100, 1)

        clear_progressbar.configure(value=progressValue)
        clear_progress.configure(text=f'{progressValue}%')

        if progressValue == 100:
            tkinter.messagebox.showinfo(title='Done', message='Text was deleted')
...
def create_video():
    ...
    voices = ['./audio_files_raw/{}'.format(f) for f in listdir('./audio_files_raw') if isfile(join('./audio_files_raw', f))]
    images = natsorted(list(['./images/{}'.format(f) for f in listdir('./images') if isfile(join('./images', f))]))
...
video_images_label = customtkinter.CTkLabel(master=generate_video, text="Select Images Path", font=("Roboto", 15, 'bold'))
video_images_label.pack(pady=10)

video_image_entry = customtkinter.CTkEntry(master=generate_video, width=270, font=("Roboto", 15))
video_image_entry.configure(state=DISABLED)
video_image_entry.pack(pady=10)

video_image_location = customtkinter.CTkButton(master=generate_video, text='Set Location', font=("Roboto", 15), command=get_video_images_location)
video_image_location.pack(pady=10)

video_voice_label = customtkinter.CTkLabel(master=generate_video, text="Select Voices Path", font=("Roboto", 15, 'bold'))
video_voice_label.pack(pady=10)

video_audios_entry = customtkinter.CTkEntry(master=generate_video, width=270, font=("Roboto", 15))
video_audios_entry.configure(state=DISABLED)
video_audios_entry.pack(pady=10)

video_audio_location = customtkinter.CTkButton(master=generate_video, text='Set Location', font=("Roboto", 15), command=get_voices_location)
...
def video_start():
    ...
    if not video_images_location or  not voices_location:
        tkinter.messagebox.showerror(title='Error', message='Please set images and video location location')
        return
    
    thread = threading.Thread(target=create_video)
    thread.start()

video_start_btn = customtkinter.CTkButton(master=generate_video, text='Start', font=("Roboto", 20, 'bold'), width=220, height=40, command=video_start)
...
video_progress = customtkinter.CTkLabel(master=generate_video, text='0%', font=("Roboto", 16, 'bold'))
video_progress.pack(pady=5)

# Others

clear_label = customtkinter.CTkLabel(master=clear_text, text="Clear Text", font=("Roboto", 24, 'bold'))
clear_label.pack(pady=15, padx=10)

clear_entry = customtkinter.CTkEntry(master=clear_text, width=270, font=("Roboto", 15))
clear_entry.configure(state=DISABLED)
clear_entry.pack(pady=10)

clear_location = customtkinter.CTkButton(master=clear_text, text='Import Images', font=("Roboto", 15), command=import_images)
clear_location.pack(pady=10)

language_label = customtkinter.CTkLabel(master=clear_text, text="Select Language", font=("Roboto", 15, 'bold'))
language_label.pack(pady=10)

language_menu = customtkinter.CTkComboBox(master=clear_text, width=270, values=["English", "Japanese", "Korean", "Russian"])
language_menu.pack(pady=10)

clear_slider_label = customtkinter.CTkLabel(master=clear_text, text="Confidence: 0.15", font=("Roboto", 15, 'bold'))
clear_slider_label.pack(pady=10)

clear_slider = customtkinter.CTkSlider(master=clear_text, from_=0, to=1, number_of_steps=100, command=set_confidence)
clear_slider.pack()

clear_start_btn = customtkinter.CTkButton(master=clear_text, text='Start', font=("Roboto", 20, 'bold'), width=220, height=40, command=clear_start)
...
clear_progress = customtkinter.CTkLabel(master=clear_text, text='0%', font=("Roboto", 16, 'bold'))
